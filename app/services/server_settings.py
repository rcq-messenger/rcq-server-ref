...
_reg(SettingSpec("closed_island", "bool", lambda: False, "limits",
                 ...
                 "\u26a0\u26a0 IT ALSO CUTS THIS ISLAND OFF FROM EVERY OTHER "
                 "ONE, and that is not what most operators expect from a "
                 "privacy setting. A person on another island has no account "
                 "here, so they are a stranger by this rule, and the key they "
                 "need in order to write the very first word is withheld. No "
                 "contact request reaches your residents from outside and no "
                 "acceptance leaves. No released client presents a card on "
                 "that door yet, so there is no way around it from the other "
                 "side. It cost the flagship a day of broken federation on "
                 "2026-09-09 before anybody noticed. Leave it off until the "
                 "clients ship guest cards, and charge for entry with the "
                 "registration policy instead: they are two different "
                 "products. "
