...
The island never touches money. Your **till** (a copy of the checkout worker
in `deploy/console-worker` of the rcq-infra repository) watches your wallets,
and when a transfer lands it signs an **access code** the buyer pastes into
the app. The island checks the signature, spends the code once, and marks the
account a resident.
...
host, and `UIN_VOUCHER_PRIVKEY` as its secret (the console-worker README walks
through it, including how to generate the keypair).
...
1. `curl https://<your-till>/v1/entry/quote?host=<host>` must answer your
   ...
   a key that does not match, no price, or no wallets.
